...
func TestNotificationTargetsFromLegacyLines(t *testing.T) {
	...
	want := []notificationTarget{{URL: "smtp://one.example.test", Enabled: true}, {URL: "telegram://token@telegram?channels=1", Enabled: true}}
	if len(got) != len(want) {
...
func TestNotificationTargetsFromJSON(t *testing.T) {
	got := notificationTargets(`[{"url":"smtp://one.example.test","enabled":true},{"url":"telegram://token@telegram?channels=1","enabled":false}]`)
	want := []notificationTarget{{URL: "smtp://one.example.test", Enabled: true}, {URL: "telegram://token@telegram?channels=1", Enabled: false}}
	if len(got) != len(want) {
		t.Fatalf("got %v, want %v", got, want)
	}
	for i := range want {
...
func TestEnabledNotificationURLs(t *testing.T) {
	got := enabledNotificationURLs(`[{"url":"smtp://one.example.test","enabled":true},{"url":"telegram://token@telegram?channels=1","enabled":false}]`)
