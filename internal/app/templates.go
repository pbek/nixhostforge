...
<section class="panel"><h2>Hosts</h2><div class="host-grid">{{range .Hosts}}<article class="host-card {{.LastStatus}}"><h3>{{.Name}}</h3><p>enabled</p><p>Last: {{if .LastStatus}}{{.LastStatus}}{{else}}no build{{end}}</p></article>{{else}}<p class="muted">No enabled hosts to show.</p>{{end}}</div></section>
