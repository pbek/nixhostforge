...
<section class="panel"><h2>Hosts</h2><div class="host-grid">{{range .Hosts}}<article class="host-card {{.LastStatus}}"><h3>{{.Name}}</h3><p>{{if .Enabled}}enabled{{else}}disabled{{end}}</p><p>Last: {{if .LastStatus}}{{.LastStatus}}{{else}}no build{{end}}</p></article>{{else}}<p class="muted">No hosts discovered yet. Trigger a check after configuring a repository.</p>{{end}}</div></section>
